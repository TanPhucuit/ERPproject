...
const insertRowsWithFallback = async (table: string, variants: any[][]) => {
  ...
  for (const rows of variants) {
    ...
    const { error } = await supabase.from(table).insert(stripGeneratedRows(table, rows))
    if (!error) return
    lastError = error
  }
  throw lastError
...
const insertOneWithFallback = async (table: string, variants: Record<string, any>[]) => {
  ...
  for (const payload of variants) {
    const { data, error } = await supabase.from(table).insert(stripGeneratedColumns(table, payload)).select().single()
...
const generatedColumnsByTable: Record<string, string[]> = {
  quotations: ['total_amount'],
  sales_orders: ['total_amount', 'estimated_profit', 'profit_margin_percent'],
  purchase_orders: ['total_amount'],
  customer_invoices: ['total_amount', 'outstanding_amount'],
  vendor_bills: ['total_amount', 'outstanding_amount'],
  quotation_lines: ['line_total'],
  sales_order_lines: ['line_total', 'line_cost'],
  purchase_order_lines: ['line_total'],
  goods_receipt_lines: ['line_total'],
  customer_invoice_lines: ['line_total'],
  vendor_bill_lines: ['line_total'],
  credit_note_lines: ['line_total'],
  debit_note_lines: ['line_total'],
  stock_levels: ['quantity_available'],
  inventory_adjustment_lines: ['variance'],
}

const stripGeneratedColumns = (table: string, payload: Record<string, any>) => {
  const generatedColumns = generatedColumnsByTable[table] || []
  if (generatedColumns.length === 0) return payload
  const clean = { ...payload }
  generatedColumns.forEach(column => delete clean[column])
  return clean
}

const stripGeneratedRows = (table: string, rows: Record<string, any>[]) =>
  rows.map(row => stripGeneratedColumns(table, row))
...
const normalizeWriteBody = async (pathname: string, body: Record<string, any>) => {
  ...
  if (pathname === '/sales-orders/quotations' || pathname.startsWith('/sales-orders/quotations/')) {
    ...
    return {
      ...
      // tax_percent is the only editable financial field; total_amount is generated by DB
  ...
  if (pathname === '/sales-orders' || pathname.startsWith('/sales-orders/')) {
    ...
    return {
      ...
      // Only tax_percent is editable; total_amount/profit are generated by DB
      subtotal: amounts.subtotal,
      tax_percent: amounts.taxPercent || 10,
      tax_amount: amounts.taxAmount,
...
const writeResource = async <T>(path: string, body: Record<string, any>, method: 'POST' | 'PUT') => {
  ...
  const upsert = async (table: string, id?: string): Promise<any> => {
    const writePayload = stripGeneratedColumns(table, normalizedBody)
    const query =
      method === 'POST'
        ? supabase.from(table).insert(writePayload).select().single()
        : supabase.from(table).update(writePayload).eq('id', id).select().single()
