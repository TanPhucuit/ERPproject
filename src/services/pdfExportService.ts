...
export const exportVendorBillToPDF = (bill: VendorBillData) => {
  const billNumber = bill.bill_number || bill.billNumber || 'BILL-000'
  const supplierName = bill.supplier?.supplier_name || bill.supplier?.name || bill.supplier_name || bill.supplierName || 'Supplier'
  const lines = bill.items?.length ? bill.items : bill.lines || []
  const subtotal = bill.subtotal || lines.reduce(
    (sum, item) => sum + (item.line_total || item.total || (item.unit_price || 0) * (item.quantity_ordered || item.quantity || 1)),
    0
  )
  const storedTaxAmount = bill.tax_amount || bill.taxAmount || 0
  const taxAmount = storedTaxAmount > 0 ? storedTaxAmount : Math.round(subtotal * 0.1)
  const debitAmount = bill.adjustmentAmount || 0
  const totalBeforeDebit = subtotal + taxAmount
  const totalAmount = bill.amountDue ?? Math.max(totalBeforeDebit - debitAmount, 0)
  const billDate = bill.issue_date || bill.billDate || bill.created_at?.split('T')[0] || new Date().toISOString().split('T')[0]
  ...
  const rows = lines.length > 0
    ? lines.map((item) => `
      <tr>
        <td>${item.description || item.product_name || item.product_sku || 'Item'}</td>
        <td class="right">${item.quantity_ordered || item.quantity || 1}</td>
        <td class="right">${money(item.unit_price || 0)}</td>
        <td class="right">${money(item.line_total || item.total || (item.unit_price || 0) * (item.quantity_ordered || item.quantity || 1))}</td>
      </tr>
    `).join('')
    : '<tr><td colspan="4" class="empty">No line items</td></tr>'
      ...
      <div class="invoice-container">
        ...
        <div class="invoice-info">
          <div>
            ...
            <p><strong>Due Date:</strong> ${dueDate || '-'}</p>
            <p><strong>Status:</strong> ${(bill.status || 'posted').toUpperCase()}</p>
          </div>
          <div>
            <h3>Vendor</h3>
        ...
        <div class="summary">
          <div class="summary-row"><span>Subtotal</span><strong>${money(subtotal)}</strong></div>
          <div class="summary-row"><span>Tax 10%</span><strong>${money(taxAmount)}</strong></div>
          ${debitAmount > 0 ? `<div class="summary-row debit"><span>Debit Note${bill.adjustmentReason ? ` (${bill.adjustmentReason})` : ''}</span><strong>-${money(debitAmount)}</strong></div>` : ''}
          <div class="summary-row total"><span>Total Amount</span><strong>${money(totalAmount)}</strong></div>
        </div>
        ${bill.notes ? `<div class="notes"><strong>Notes:</strong><p>${bill.notes.replace(/\n/g, '<br>')}</p></div>` : ''}
        <div class="footer">
          <p>Generated on ${new Date().toLocaleString('vi-VN')}</p>
        </div>
      </div>
  ...
  if (printWindow) {
    ...
    alert('Please disable popup blockers to print/download vendor bill')
  }
}
